...
// One fixtures file per test, under data/qtest/. chdir is the server package
// (see BUILD.bazel), like geo.test.ts. New files are produced by the
// record_departures tool.
async function loadFixtures(file: string): Promise<fetcher.Fixtures> {
...
async function newQ(file: string): Promise<{
    q: queryDepartures.Q;
...
const tests = [
    testAsync("normalizes departures for a single stop", async () => {
    ...
    testAsync("discovers lines when none are requested", async () => {
    ...
    testAsync("makes a second request near midnight and dedupes", async () => {
    ...
    testAsync("filters past departures and sorts across stops", async () => {
    ...
    testAsync("builds the get_departures URL with base64 lines", async () => {
    ...
    testAsync("queries available_lines before get_departures", async () => {
        const { q, fetcher: f } = await newQ("discovery.json");
        await q.getDepartures(
            { stopGid: "de:09162:2", lines: [], timeToStop: 3 },
            new Date(1783324800 * 1000),
    ...
    testAsync("near midnight requests both hours by timestamp", async () => {
        const { q, fetcher: f } = await newQ("midnight.json");
        await q.getDepartures(
    ...
    testAsync("recorded: Karlsplatz (Stachus) line discovery", async () => {
